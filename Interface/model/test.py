...
#import align_fp.matlab_to_python.align_fp as align_fp_python
import align_fp.align_python_new.align_fp as ap
...
def get_userinfo_adjust(userRoomID,adptRoomID,NewGraph):
    ...
    e=time.time
    print(' model test time: %s Seconds')
    ...
    #boxes    = np.array(Box,dtype=int) #.tolist()
    #types    = np.array(rNode,dtype=int) #.tolist()
    #edges    = np.array(Edge,dtype=int) #.tolist()
    #image    = np.array(fp_end.data.gene.astype(float).copy(),dtype=int) #.tolist()
    #threshold = 18
    #result = ap.align_fp(np.array(boundary), np.array(boxes), np.array(types), edges, np.array(image), np.array(threshold), False)
    #print("    #test matlab to python          ")
    #print(result)
    Boundary = np.array(boundary)
    box = np.array(Box)
    rnode = np.array(rNode)
    edge = np.array(Edge)
    image    = np.array(fp_end.data.gene.astype(float).copy(),dtype=int)
    threshold = 18
    drawResult = False

    # Gọi hàm align_fp với các tham số đã chuẩn bị
    #newBox, order, rboundary = ap.align_fp(Boundary, box, rnode, edge, image, threshold, drawResult)
    #print("New Box Coordinates:\n", newBox)
    #print("Order of Rooms:\n", order)
    #print("Room Boundaries:\n", rboundary)



    #endcom = time.time
    print(' matlab.compute time: %s Seconds')
    ...
    return fp_end,box_out,box_order, gene_layout, boxes_refeine, rBoundary, boundary, Box, rNode, Edge, fp_end, box_refine

def align_info_python(boundary, Box, rNode, Edge, fp_end, box_refine):
    boundary = np.array(boundary,dtype=int) #.tolist()
    boxes    = np.array(Box,dtype=int) #.tolist()
    types    = np.array(rNode,dtype=int) #.tolist()
    edges    = np.array(Edge,dtype=int) #.tolist()
